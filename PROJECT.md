...
| **v0.36 P5** | **DEFERRED — needs live services / an LLM key.** Real external brand & company search, live domain + trademark integrations, a dedicated **Brand Mode** (collision-aware, brand-safe output distinct from meaning-discovery mode, §25), and the **LLM benchmark suite** (§21: direct-answer precision, drift rate, duplicate rate, false-exceptional rate, collision false-negative rate, etc.). Blocked in the sandbox: outbound APIs are proxy-restricted and there is no `ANTHROPIC_API_KEY`, so these cannot be exercised here. The offline half of collision (P4) is shipped; the layered `collisionReport` already exposes the seams (`brand`/`domain`/`trademark`/`multilingual` = `not_checked`) that live checks will fill. Resume when deployed with keys + network, or to scaffold the deterministic parts (Brand Mode heuristics, benchmark harness) on request. | ⏳ waiting (live services / key) |
