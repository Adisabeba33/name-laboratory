...
| **DEDUPE** | **Proof of meaning — lexicon de-duplication (v0.52).** First on-device step toward a collaborative dictionary (see §7b): before a word joins the personal lexicon it is checked against what's already saved so the collection never holds the same idea twice. `src/lib/dedupe.ts` (pure, deterministic, no network) runs two checks — **form** (edit distance + the shared `phoneticKey`, so "Varethis" ≈ "Varethas" and "Kwik" ≈ "Quick") and **meaning** (token-overlap Jaccard of meaning+brief, compared ONLY across a *different* concept — same-prompt variants are meant to be alike and aren't flagged). `App.toggleSave` runs it before committing; a hit opens `DuplicateDialog` ("may already be in your lexicon", with the close entries + add-anyway/cancel). The same logic is designed to move server-side against the shared dictionary later. 5 regression tests. 140 green. | 🟢 shipped (personal lexicon) |
| **AUTH** | **Accounts — Stage 1: sign-in (v0.53).** First step of the collaborative-dictionary backend. `src/lib/auth.ts` wraps Supabase Auth (email magic-link + Google), the SDK **lazy-loaded** (dynamic import) only when configured so a guest never downloads it. `AuthDialog` (passwordless email flow) + a Sidebar sign-in/out chip; `App` tracks the session via `onAuthChange`. **Entirely optional:** with no `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` the app runs exactly as before (guest + localStorage) — the sign-in UI simply doesn't appear. Nothing syncs yet (that's Stage 2). Env: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY` (client), `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (server, for Stage 2). 140 tests green. | 🟡 needs Supabase keys to activate |
| **AUTH2** | **Accounts — Stage 2: cloud lexicon sync (v0.54).** Signed-in users' saved words live in Supabase Postgres and sync across devices. Architecture: the browser talks to Postgres **directly** via supabase-js, and **row-level security** (`supabase/schema.sql`, table `lexicon_entries`, `auth.uid() = user_id` policies) guarantees each account only ever reads/writes its OWN rows — so the private lexicon needs no bespoke `/api` (that's reserved for Stage 3's server-side dedupe). `src/lib/lexicon-cloud.ts` (`toRow`/`fromRow` + `cloudList/cloudUpsert/cloudRemove`, all no-ops for a guest). `App`: on sign-in, migrate any local words up then load the cloud copy; every save/remove writes through `persistAdd`/`persistRemove` (localStorage always + cloud when signed in). Still fully inert without keys (guest + localStorage, identical to before). `supabase/schema.sql` is run once by the user in the Supabase SQL editor. 2 mapping tests; 145 green. | 🟡 needs Supabase keys + schema.sql |
| A | **Accounts + database** — Stage 3 shared dictionary + server-side dedupe (via `/api`), Stage 4 authorship/points (see §7a) | ⏳ next |
